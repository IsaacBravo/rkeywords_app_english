...
clean_corpus <- function(seedWords, corpus, minSentenceChar = 3) {
  ...
  findSeed <- stringr::str_detect(as.character(corpus), stringr::regex(seedRegex, ignore_case = TRUE))
  shortCorp <- quanteda::corpus_subset(corpus, findSeed)

  # Split sentences
  sentence_tokens <- spacyr::spacy_tokenize(shortCorp, what = "sentence", output = "data.frame")
  names(sentence_tokens) <- c("orig_doc", "sentence")

  # Sentence Clean
  sentLength <- sapply(sentence_tokens$sentence, stringr::str_length)
